...
function firstpress_scripts() {
	
	wp_enqueue_style( 'firstpress-style', get_stylesheet_uri() );

	wp_enqueue_script( 'firstpress-core-js', get_template_directory_uri() . '/inc/js/compiled.js', array('jquery'), true); 
	
}

add_action( 'wp_enqueue_scripts', 'firstpress_scripts' );
...
wp_add_dashboard_widget('custom_help_widget', 'firstpress Support', 'custom_dashboard_help');
}
function custom_dashboard_help() {
?>

<img src="https://firstpress.co.uk/wp-content/themes/firstpress/images/logo__firstpress.svg" style="max-width:100%;
height:auto;"/>

<img src="https://firstpress.co.uk/wp-content/uploads/2016/10/icon-screen-delete.svg" style=" display: inline-block; width: 60px; margin: 2em calc(50% - 30px) 1em;"/>

<p>For support or general enquiries please contact us directly at <a href="mailto:hello@firstpress.co.uk">hello@firstpress.co.uk</a> or call <a href="tel:+44 (0)1672 556532">01672 556532</a></p>
...
function firstpress_social_sharing_buttons($content) {
	global $post;
	if(is_singular() || is_home()){
	
		// Get current page URL 
		$firstpressURL = urlencode(get_permalink());
 
		// Get current page title
		$firstpressTitle = htmlspecialchars(urlencode(html_entity_decode(get_the_title(), ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8');
		// $firstpressTitle = str_replace( ' ', '%20', get_the_title());
		
		// Get Post Thumbnail for pinterest
		$firstpressThumbnail = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
 
		// Construct sharing URL without using any script
		$twitterURL = 'https://twitter.com/intent/tweet?text='.$firstpressTitle.'&amp;url='.$firstpressURL.'&amp;via=Crunchify';
		$facebookURL = 'https://www.facebook.com/sharer/sharer.php?u='.$firstpressURL;
 
		// Based on popular demand added Pinterest too
		$pinterestURL = 'https://pinterest.com/pin/create/button/?url='.$firstpressURL.'&amp;media='.$firstpressThumbnail[0].'&amp;description='.$firstpressTitle;
